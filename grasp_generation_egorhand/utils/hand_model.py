...
class HandModel:
    def __init__(self, mjcf_path, mesh_path, contact_points_path, penetration_points_path, n_surface_points=0, device='cpu'):
        ...
        def build_mesh_recurse(body):
                ...
                for visual in body.link.visuals:
                    ...
                    if visual.geom_type == "box":
                        ...
                        if visual.geom_param is not None:
                            ...
                            print(f"Warning: 'box' visual.geom_param is None for link {link_name}")
                        ...
                        link_mesh = tm.load_mesh(os.path.join(mesh_path, visual.geom_param[0].split(":")[1]+".STL"), process=False)
            ...
            # if body.link.visuals:
            #     visual = body.link.visuals[0]  # предполагаем, что берём первый визуальный элемент

            #     if hasattr(visual, "mesh") and visual.mesh is not None:
            #         mesh_name = visual.mesh.name  # например, "Link_left_abduction"
            #     elif visual.geom_param and visual.geom_param[0]:
            #         mesh_name = visual.geom_param[0]  # например, "base_link"
            #     else:
            #         raise ValueError(f"Cannot determine mesh for visual element: {visual}")

            #     mesh_file = mesh_name + ".STL"
            #     full_path = os.path.join(self.mesh_path, mesh_file)
            #     link_mesh = tm.load_mesh(full_path, process=False)
        ...
        def set_joint_range_recurse(body):
            if body.joint.joint_type != "fixed":
                print(f"Found joint: {body.joint.name} — range: {body.joint.range}")
                self.joints_names.append(body.joint.name)
                # if body.joint is not None and body.joint.range is not None:
    ...
    def cal_distance(self, x):
        ...
        for link_name in self.mesh:
            ...
            '''Расчёт расстояния от точки до геометрии звена'''
            '''Если у звена есть сложная меш-сетка (например, base_link), используется приближённый SDF (Signed Distance Function) через compute_sdf'''
            if 'geom_param' not in self.mesh[link_name]:
                ...
                nearest_point = x_local.detach().clone() # ближайшая точка на оси капсулы.
                nearest_point[:, :2] = 0
                nearest_point[:, 2] = torch.clamp(nearest_point[:, 2], 0, height)
                dis_local = radius - (x_local - nearest_point).norm(dim=1)
            
            '''Добавляем расстояния к списку'''
            dis.append(dis_local.reshape(x.shape[0], x.shape[1]))

        '''берём максимальное расстояние по всем звеньям '''    
        dis = torch.max(torch.stack(dis, dim=0), dim=0)[0]

        '''Возвращает тензор формы (B, N), где каждое значение — подписанное расстояние до поверхности руки'''
        return dis
