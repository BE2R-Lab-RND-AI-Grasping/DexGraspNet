...
def main():
    ...
    for i, pose_i in enumerate(pose_dict):
        ...
        try:
            ...
            # Более информативная визуализация в вашем коде:
            plt.figure(figsize=(12, 5))
            for i, joint_name in enumerate(validator.joint_finger_names):
                plt.plot(np.rad2deg(control_error_fingers[:, i]), label=joint_name, alpha=0.7)

            plt.xlabel('Simulation step')
            plt.ylabel('Position error (degrees)')
            plt.title(f'Joint position error | Stable grasp: {is_stable}')
