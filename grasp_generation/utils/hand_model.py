...
class HandModel:
    def __init__(self, hand_config, mjcf_path, mesh_path, contact_points_path, penetration_points_path, n_surface_points=0, device='cpu'):
        ...
        self.device = device
         ...
         # Добавляем сбор информации о типах суставов
        self.joint_types = []  # 'rot' для вращательных, 'lin' для линейных
        ...
        def collect_joint_types_recurse(body):
            if body.joint.joint_type != "fixed":
                if body.joint.joint_type == "slide":
                    self.joint_types.append("lin")  # линейный сустав
                else:
                    self.joint_types.append("rot")  # вращательный сустав (по умолчанию)
            for children in body.children:
                collect_joint_types_recurse(children)
        
        collect_joint_types_recurse(self.chain._root)
    ...
    def set_parameters(self, hand_pose, contact_point_indices=None):
        ...
        # print(f"self.hand_pose[:, 9:]: {self.hand_pose[:, 9:]}")
        ...
        # # Разделяем параметры суставов по типам
        # joint_params = self.hand_pose[:, 9:]

        # # Создаем список трансформаций для каждого сустава
        # joint_transforms = []
        # for i, (param, j_type) in enumerate(zip(joint_params.T, self.joint_types)):
        #     if j_type == "lin":
        #         # Линейный сустав - создаем трансформацию сдвига
        #         # (предполагаем движение вдоль локальной оси X)
        #         batch_size = param.shape[0]
        #         transform = torch.eye(4, device=param.device).unsqueeze(0).repeat(batch_size, 1, 1)
        #         transform[:, 0, 3] = param  # перемещение вдоль X
        #         joint_transforms.append(transform)
        #     else:
        #         # Вращательный сустав - создаем трансформацию вращения
        #         batch_size = param.shape[0]
        #         transform = torch.eye(4, device=param.device).unsqueeze(0).repeat(batch_size, 1, 1)
        #         cos_t = torch.cos(param)
        #         sin_t = torch.sin(param)
        #         transform[:, 1, 1] = cos_t
        #         transform[:, 1, 2] = -sin_t
        #         transform[:, 2, 1] = sin_t
        #         transform[:, 2, 2] = cos_t
        #         joint_transforms.append(transform)
        # print(f"JOINT_TRANSFORMS: {joint_transforms}")

        # # Передаем трансформации в forward_kinematics
        # self.current_status = self.chain.forward_kinematics(joint_transforms)
    ...
    def cal_distance(self, x):
        ...
        for link_name in self.mesh:
            ...
            # print(f"GEOM PARAM: {self.mesh[link_name]['geom_param']}")
        ...
        def build_mesh_recurse(body):
            if(len(body.link.visuals) > 0):
                ...
                for visual in body.link.visuals:
                    scale = torch.tensor([1, 1, 1], dtype=torch.float, device=device)
                    if visual.geom_type == "box":
                        extents = [visual.geom_param[0]*2, visual.geom_param[1]*2, visual.geom_param[2]*2]
                        ...
                        if visual.geom_param[1] is not None:
                            scale = torch.tensor(visual.geom_param[1], dtype=torch.float, device=device)
                    elif visual.geom_type == "ellipsoid":
                        sphere = tm.creation.icosphere(subdivisions=3, radius=1.0)
                        axes = [visual.geom_param[0], visual.geom_param[1], visual.geom_param[2]]
                        ellipsoid = sphere.copy()
           
                        ellipsoid.apply_scale(axes)
