...
def split_mesh_by_world_z(mesh: tm.Trimesh, z_percent: float = None):
    """
    Split mesh surface into two meshes by plane Z = z0 (world frame).
    If z_percent is provided, use that fraction of the mesh height instead.
    Returns (upper_mesh, lower_mesh).
    """
    if z_percent is not None:
        z_min, z_max = mesh.bounds[:, 2]
        z0 = z_min + float(z_percent) * (z_max - z_min)

    # Keep side where dot(n, x) >= origin·n
    # For upper: z >= z0
    upper = mesh.slice_plane(plane_origin=[0, 0, z0], plane_normal=[0, 0, 1])
    # For lower: z <= z0 (use opposite normal)
    lower = mesh.slice_plane(plane_origin=[0, 0, z0], plane_normal=[0, 0, -1])
    return upper, lower

    
def split_mesh_by_cone(mesh: tm.Trimesh, height: float, axis: np.ndarray, angle_deg: float):
    radius = np.tan(np.radians(angle_deg/2)) * height
    transform = np.eye(4)
    transform[:3, :3] = tm.transformations.rotation_matrix(math.atan2(axis[1], axis[0]), [0, 0, 1])[:3, :3]
    cone = tm.creation.cone(radius=radius, height=height, sections=32, transform=transform) 
 
    return tm.boolean.intersection([mesh, cone], engine='blender')
 

def initialize_convex_hull(init_pos, hand_model, object_model, args, is_z_split = True):
    """
    ...
    for i in range(n_objects):
        ...
        rotation_hand = torch.tensor(transforms3d.euler.euler2mat(0, 0, 0, axes='rzxz'), dtype=torch.float, device=device)
    ...
    joint_angles_mu = torch.tensor(init_pos, dtype=torch.float, device=device)
