            ...
            if (len(body.link.visuals) > 0):
                ...
                for visual in body.link.visuals:
                    ...
                    if visual.geom_type == "box":
                        ...
                        if visual.geom_param and len(visual.geom_param) > 0:
                            link_mesh = trimesh.load_mesh(os.path.join(mesh_path, visual.geom_param[0] + ".STL"), process=False)
                        else:
                            print(f"Warning: geom_param is empty for visual {visual}")
